...
public:
    ...
    virtual ~tridiagonal_matrix() { std::cout << "destructor called\n"; }

    std::unique_ptr<tridiagonal_matrix> clone() const
    {
        auto down = std::vector<double>(m_down);
        auto upper = std::vector<double>(m_upper);
        auto middle = std::vector<double>(m_middle);
        return std::make_unique<tridiagonal_matrix>(down, upper, middle);
    }
};
